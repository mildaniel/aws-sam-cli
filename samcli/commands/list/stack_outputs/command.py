...
from samcli.commands.list.cli_common.options import list_common_options
...
def cli(self, stack_name, output, query, config_file, config_env):
    """
    `sam list stack-outputs` command entry point
    """
    do_cli(stack_name=stack_name, output=output, query=query, region=self.region, profile=self.profile)


def do_cli(stack_name, output, query, region, profile):
    """
    ...
    with StackOutputsContext(
        stack_name=stack_name, output=output, region=region, profile=profile, query=query
